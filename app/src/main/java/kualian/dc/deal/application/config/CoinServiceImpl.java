...
import kualian.dc.deal.application.ui.Configuration;
...
//                wakeLock.release();
    ...
    private ServerClients getServerClients(Wallet wallet) {
        ServerClients newClients = new ServerClients(WalletConstants.DEFAULT_COINS_SERVERS, connHelper);
        if (application.getTxCachePath() != null) {
            newClients.setCacheDir(application.getTxCachePath(), WalletConstants.TX_CACHE_SIZE);
        }
        return newClients;
    }
    ...
    private final BroadcastReceiver tickReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            if (lastStop > 0) {
                ...
                if (secondsIdle > WalletConstants.STOP_SERVICE_AFTER_IDLE_SECS) {
    ...
    public int onStartCommand(final Intent intent, final int flags, final int startId) {
        ...
        if (CoinService.ACTION_CANCEL_COINS_RECEIVED.equals(action)) {
            ...
            if (application.getWallet() != null) {
                Wallet wallet = application.getWallet();
                if (intent.hasExtra(WalletConstants.ARG_ACCOUNT_ID)) {
                    String accountId = intent.getStringExtra(WalletConstants.ARG_ACCOUNT_ID);
            ...
            if (application.getWallet() != null) {
                Wallet wallet = application.getWallet();
                if (intent.hasExtra(WalletConstants.ARG_ACCOUNT_ID)) {
                    lastAccount = intent.getStringExtra(WalletConstants.ARG_ACCOUNT_ID);
